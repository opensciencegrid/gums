...
public class Pool_AddRange extends RemoteCommand {
    ...
    protected void execute(org.apache.commons.cli.CommandLine cmd)
        ...
        for (int nArg = 2; nArg < cmd.getArgs().length; nArg++) {
            addRange(persistenceManager, groupName,
                cmd.getArgs()[nArg]);
        }
    }

    private void addRange(String persistenceManager, String groupName, String range) throws Exception {
        ...
        for (int account = nFirstAccount; account <= nLastAccount; account++) {
            ...
            getGums().poolAddAccount(persistenceManager, groupName, buf.toString());
