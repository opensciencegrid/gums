 ...
 */
public class GenerateEmailMapfile extends GenerateMap {
    static {
        command = new GenerateEmailMapfile();
    }

    /**
     * Creates a new GenerateGridMapfile object.
     */
    public GenerateEmailMapfile() {
        syntax = "[-g GUMSURL] [-f FILENAME] [SERVICEDN]";
        description = "Generates the email-mapfile for a service/host. ";
...
//            "When using ./bin/gums-host, SERVICEDN defaults to the host certificate DN.";
    ...
    protected String generateMap(String hostname, String gumsUrl) throws Exception {
        String map = getGums(gumsUrl).generateEmailMapfile(hostname);
        if (map == null) {
            System.err.println("Could not create Email-map-file.");
