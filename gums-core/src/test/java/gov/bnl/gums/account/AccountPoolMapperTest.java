...
public class AccountPoolMapperTest extends TestCase {
    ...
    public void testMapUser() {
        ...
        String account = mapper.mapUser("/DC=org/DC=griddev/OU=People/CN=User "+i, true).getUser();
        while (account != null) {
            i++;
            assertFalse(accounts.contains(account));
            accounts.add(account);
            account = mapper.mapUser("/DC=org/DC=griddev/OU=People/CN=User "+i, true).getUser();
        }
    }
    
    public void testMapUser2() {
        String account1 = mapper.mapUser("/DC=org/DC=griddev/OU=People/CN=User 1", true).getUser();
        String account2 = mapper.mapUser("/DC=org/DC=griddev/OU=People/CN=User 2", true).getUser();
        assertEquals(account1, mapper.mapUser("/DC=org/DC=griddev/OU=People/CN=User 1", true).getUser());
        assertEquals(account2, mapper.mapUser("/DC=org/DC=griddev/OU=People/CN=User 2", true).getUser());
        assertEquals(account2, mapper.mapUser("/DC=org/DC=griddev/OU=People/CN=User 2", true).getUser());
        assertEquals(account1, mapper.mapUser("/DC=org/DC=griddev/OU=People/CN=User 1", true).getUser());
    }
